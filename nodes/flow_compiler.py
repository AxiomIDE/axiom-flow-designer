...
def flow_compiler(log: AxiomLogger, secrets: AxiomSecrets, input: FlowBuildContext) -> FlowBuildContext:
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            input.compile_success = False
            input.compile_error = f"BFF compile returned {resp.status_code}: {resp.text[:500]}"
    except Exception as e:
        log.error(f"FlowCompiler failed: {e}")
        input.compile_success = False
