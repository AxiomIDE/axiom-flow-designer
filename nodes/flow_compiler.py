...
def flow_compiler(log: AxiomLogger, secrets: AxiomSecrets, input: FlowSpec) -> CompileResult:
    """Compile the flow graph via the BFF."""
    ...
    if not input.graph_json:
        return CompileResult(
            success=False,
            error="No graph_json to compile",
        )
    ...
    try:
        ...
        if resp.status_code == 200:
            ...
            return CompileResult(
                success=False,
                error=f"BFF compile returned {resp.status_code}: {resp.text[:500]}",
                graph_json=input.graph_json,
            )
    except Exception as e:
        log.exception("FlowCompiler failed")
